...
def transform_coordinates(current_pose, current_frame, target_frame):
    """计算current_pose 在 current_frame坐标系下的位置姿态，可以理解为一个偏移
    然后将该偏移姿态叠加在 target_frame上，得到新的姿态"""
    current_frame = np.ascontiguousarray(current_frame)# A_T_B
    current_pose = np.ascontiguousarray(current_pose)# A_T_C
    target_frame = np.ascontiguousarray(target_frame) # X_T_D
    # 计算偏移量 B_T_C
